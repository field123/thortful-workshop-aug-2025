...
export class CartService {
  ...
  public async fetchCart(): Promise<void> {
    ...
    try {
      ...
      const items: CartItemsObjectResponse[] = included?.items || [];
  ...
  public async addToCart(productId: string, quantity: number = 1, options?: {
    ...
    try {
      ...
      if (options?.customInputs) {
        body.data.custom_inputs = options.customInputs;
      }
      
      if (options?.bundleConfiguration) {
        body.data.bundle_configuration = options.bundleConfiguration;
      }
      
      if (options?.location) {
        body.data.location = options.location;
      }
