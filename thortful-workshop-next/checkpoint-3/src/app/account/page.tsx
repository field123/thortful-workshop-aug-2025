...
/*
    TUTORIAL STEP: Insert the account data fetch here
*/
...
export default async function AccountPage() {
  ...
  /*
    TUTORIAL STEP: Insert the subscriptions data fetch here
  */
          ...
          <div className={styles.detailsCard}>
            <div className={styles.detailRow}>
              ...
              <span className={styles.value}>
                {accountData.member?.name || "Not set"}
              </span>
            </div>
            <div className={styles.detailRow}>
            ...
            <div className={styles.detailRow}>
              ...
              <span className={styles.value}>
                {accountData.account?.type || "Standard"}
              </span>
            </div>
            <div className={styles.detailRow}>
            ...
            <div className={styles.subscriptionsGrid}>
              {subscriptions.data.data
                ...
                .map((subscription: any) => {
                  ...
                  const plan = subscriptions.data?.included?.plans?.find(
                    (p: any) => p.id === planId
                  );

                  // Get the pricing option using the pricing_option_id from subscription attributes
                  const pricingOptionId =
                    subscription.attributes?.pricing_option_id;
                  const pricingOption =
                    subscriptions.data?.included?.pricing_options?.find(
                      (p: any) => p.id === pricingOptionId
                    );
                  ...
                  if (pricingOption && plan) {
                    ...
                    if (priceData?.display_price?.without_tax?.formatted) {
                      formattedPrice =
                        priceData.display_price.without_tax.formatted;
                    } else if (priceData?.display_price?.with_tax?.formatted) {
                      formattedPrice =
                        priceData.display_price.with_tax.formatted;
                    }

                    // Add interval
                    const interval =
                      pricingOption.attributes?.billing_interval_type ||
                      "month";
                    const frequency =
                      pricingOption.attributes?.billing_frequency || 1;
                    const intervalText =
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className={styles.subscriptionDetails}>
                        ...
                        {subscription.meta?.pending &&
                          subscription.attributes?.go_live_after && (
                            <p className={styles.nextBilling}>
                              Starts:{" "}
                              {new Date(
                                subscription.attributes.go_live_after
