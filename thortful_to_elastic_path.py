...
class ElasticPathConverter:
    ...
    def convert(self, input_file: str) -> str:
        ...
        # Write CSV
        headers = ['external_ref', 'name', 'sku', 'slug', 'commodity_type', 'description', 'status', 'tags', 'main_image_id']
...
def main():
    ...
    parser.add_argument('command', choices=['extract', 'convert', 'upload-images', 'import', 'all'],
                        help='Command to run')
    ...
    if args.command in ['upload-images', 'import', 'all']:
    ...
    if args.command == 'extract':
        ...
        converter = ElasticPathConverter()
        csv_file = converter.convert(json_file)
        ...
        token = auth.get_access_token()
        uploader = ElasticPathImageUploader(auth.api_url, token)
        csv_with_images = uploader.upload_all(json_file, csv_file)
